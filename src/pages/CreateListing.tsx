...
const PROPERTY_TYPES: PropertyType[] = ['1+kk','1+1','2+kk','2+1','3+kk','3+1','4+kk','4+1','atypical']
  ...
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none' }}>
      <div onClick={() => onChange(!checked)} style={{
        ...
        background: checked ? 'var(--c-accent)' : 'var(--c-border-md)',
...
export function CreateListingPage({ onDone }: { onDone: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const file of form.images) {
        const compressed = file // client-side compression can be added later
        const path = `${user.id}/${Date.now()}_${file.name.replace(/\s/g, '_')}`
        const { error: upErr } = await supabase.storage.from('listing-images').upload(path, compressed)
        if (upErr) throw upErr
        imagePaths.push(path)
      }

      // Insert listing — location as WKT for PostGIS
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--c-bg)' }}>
      ...
      <div style={{
        ...
        padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 0, flexShrink: 0,
      }}>
        {STEPS.map((s, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{
                ...
                background: i < step ? '#dcfce7' : i === step ? 'var(--c-accent)' : 'var(--c-bg)',
                color: i < step ? '#15803d' : i === step ? 'white' : 'var(--c-faint)',
                border: i === step ? 'none' : '1px solid var(--c-border)',
                flexShrink: 0,
              }}>
                {i < step ? '✓' : i + 1}
              </div>
        ...
        <div style={{ maxWidth: 540, margin: '0 auto' }}>
          ...
          {step === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={label}>Dispozice *</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                {form.lat
                  ...
                  : <p style={{ fontSize: 11, color: 'var(--c-faint)', marginTop: 6 }}>Klikni na mapu pro označení polohy bytu</p>
                }
              </div>
            </div>
          )}
      ...
      <div style={{
        ...
        background: 'var(--c-surface)', display: 'flex', justifyContent: 'space-between',
        flexShrink: 0,
      }}>
        <button onClick={step === 0 ? onDone : () => setStep(s => s - 1)} style={{
        ...
        {step < 4
          ...
          : <button onClick={handleSubmit} disabled={uploading} style={{
              ...
              {uploading ? 'Odesílám...' : '✓ Odeslat ke kontrole'}
            </button>
        }
      </div>
    </div>
